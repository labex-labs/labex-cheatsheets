---
title: 'Шпаргалка по Nmap'
description: 'Изучите Nmap с нашей исчерпывающей шпаргалкой, охватывающей основные команды, концепции и лучшие практики.'
...
Изучите сетевое сканирование Nmap с помощью практических лабораторий и реальных сценариев. LabEx предлагает комплексные курсы по Nmap, охватывающие обнаружение сетей, сканирование портов, определение служб, отпечатки ОС и оценку уязвимостей. Освойте методы сетевой разведки и аудита безопасности.
</base-disclaimer-content>
</base-disclaimer>
...
Сканировать цели, перечисленные в файле.
...
Обнаружение хостов — ключевой способ, который используют многие аналитики и пентестеры с помощью Nmap. Его цель — получить обзор того, какие системы активны.
...
Используйте TCP-пакеты для обнаружения хостов.
...
Используйте UDP-пакеты для обнаружения хостов.

```bash
# UDP ping на общие порты
...
Используйте ARP-запросы для обнаружения в локальной сети.
...
```bash
# Сканирование по умолчанию (требует прав root)
nmap -sS 192.168.1.1
...
### TCP Connect-сканирование: `nmap -sT`

Nmap отправляет TCP-пакет на порт с установленным флагом SYN. Это позволяет пользователю узнать, открыты порты, закрыты или неизвестны.

```bash
# TCP connect-сканирование (права root не требуются)
nmap -sT 192.168.1.1
# Connect-сканирование с настройкой времени
nmap -sT -T3 192.168.1.1
```

### UDP-сканирование: `nmap -sU`

Сканирование UDP-портов на предмет служб.

```bash
# UDP-сканирование (медленное, требует прав root)
nmap -sU 192.168.1.1
# UDP-сканирование общих портов
...
Продвинутые методы сканирования для обхода защиты.
...
Нацеливание на определенные порты, диапазоны или комбинации TCP и UDP портов для более точного сканирования.
...
### Определение служб: `nmap -sV`

Обнаружение запущенных служб и попытка определить их программное обеспечение, версии и конфигурации.

```bash
# Базовое определение версий
nmap -sV 192.168.1.1
# Агрессивное определение версий
nmap -sV --version-intensity 9 192.168.1.1
# Легкое определение версий
nmap -sV --version-intensity 0 192.168.1.1
# Скрипты по умолчанию с определением версий
nmap -sC -sV 192.168.1.1
```

### Скрипты служб

Использование скриптов для расширенного определения служб.
...
Использование отпечатков TCP/IP для угадывания операционной системы целевых хостов.
...
Настройка скорости сканирования и скрытности в зависимости от целевой среды и риска обнаружения.
...
Тонкая настройка обработки таймаутов, повторных попыток и параллельного сканирования для оптимизации производительности.
...
Нацеливание на конкретные уязвимости или службы.
...
### Сканирование с ложными целями: `nmap -D`
...
### Манипуляция исходным IP/портом
...
### Idle/Zombie-сканирование: `nmap -sI`

Использование зомби-хоста для сокрытия источника сканирования.

```bash
# Зомби-сканирование (требуется активный хост)
...
Контроль того, как Nmap обрабатывает DNS-запросы.
...
# Показать информацию об интерфейсе и маршрутах
nmap --iflist
...
nmap --version
# Отправлять на уровне ethernet
nmap --send-eth 192.168.1.1
# Отправлять на уровне IP
...
```bash
# Шаг 1: Обнаружить активные хосты
nmap -sn 192.168.1.0/24
...
# Перечислить HTTP-службы
nmap -sS -sV --script http-* 192.168.1.50
# Проверить на распространенные уязвимости
...
Разведка с низким уровнем шума.
...
Оптимизация скорости сканирования больших сетей.

```bash
# Быстрый сетевой обход

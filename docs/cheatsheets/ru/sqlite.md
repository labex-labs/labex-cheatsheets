---
title: 'Шпаргалка по SQLite'
description: 'Изучите SQLite с нашей подробной шпаргалкой, охватывающей основные команды, концепции и лучшие практики.'
...
sqlite3 mydata.db
# Создать базу данных в памяти (временная)
sqlite3 :memory:
...
CREATE TABLE products (
    ...
    price REAL,          -- Числа с плавающей точкой
...
Определяет ограничения для обеспечения целостности данных и связей между таблицами.
...
Вставляет новые записи или обновляет существующие при конфликте.
...
Группирует строки по указанным критериям и фильтрует группы для создания сводных отчетов.
...
Объединяет данные из нескольких таблиц, используя различные типы объединений для реляционных запросов.
...
```sql
-- Установка режима журнала для лучшей производительности
...
-- Установка размера кэша (в страницах)
...
Создает виртуальные таблицы, представляющие сохраненные запросы для повторного доступа к данным.
...
-- Точки сохранения (Savepoints) для вложенных транзакций
BEGIN;
SAVEPOINT sp1;
...
-- Установка режима безопасного удаления
...
Настраивает параметры, связанные с безопасностью, и ограничения базы данных.

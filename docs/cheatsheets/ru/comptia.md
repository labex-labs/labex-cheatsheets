---
title: 'Шпаргалка CompTIA | LabEx'
description: 'Изучите ИТ-сертификации CompTIA с помощью этой комплексной шпаргалки. Краткий справочник по CompTIA A+, Network+, Security+, Linux+ и основам ИТ для подготовки к экзаменам.'
...
Изучайте сертификации CompTIA посредством практических лабораторных работ и сценариев из реального мира. LabEx предлагает комплексные курсы CompTIA, охватывающие A+, Network+, Security+ и специализированные сертификации. Освойте основы ИТ, сетевые технологии, безопасность и продвиньте свою ИТ-карьеру с помощью признанных в отрасли учетных данных.
</base-disclaimer-content>
</base-disclaimer>
...
<BaseQuiz id="comptia-core-1" correct="B">
  <template #question>
    Какая сертификация CompTIA фокусируется на основах сетей и поиске неисправностей?
  </template>
  ...
  <BaseQuizAnswer>
    CompTIA Network+ (N10-008) фокусируется на основах сетей, внедрении, эксплуатации, безопасности и поиске неисправностей. Она предназначена для сетевых администраторов и технических специалистов.
  </BaseQuizAnswer>
</BaseQuiz>
...
<BaseQuiz id="comptia-osi-1" correct="C">
  ...
  <BaseQuizOption value="A">Уровень 3 (Сетевой)</BaseQuizOption>
  <BaseQuizOption value="B">Уровень 5 (Сеансовый)</BaseQuizOption>
  <BaseQuizOption value="C" correct>Уровень 4 (Транспортный)</BaseQuizOption>
  <BaseQuizOption value="D">Уровень 7 (Прикладной)</BaseQuizOption>
  
  <BaseQuizAnswer>
    TCP (Протокол управления передачей) работает на Уровне 4 (Транспортном) модели OSI. Этот уровень отвечает за надежную передачу данных, проверку ошибок и управление потоком.
  </BaseQuizAnswer>
</BaseQuiz>
...
<BaseQuiz id="comptia-cia-1" correct="A">
  <template #question>
    Что представляет собой триада CIA в кибербезопасности?
  </template>
  
  <BaseQuizOption value="A" correct>Конфиденциальность, Целостность и Доступность — три основных принципа безопасности</BaseQuizOption>
  <BaseQuizOption value="B">Правительственное агентство</BaseQuizOption>
  <BaseQuizOption value="C">Три типа атак</BaseQuizOption>
  <BaseQuizOption value="D">Три метода аутентификации</BaseQuizOption>
  
  <BaseQuizAnswer>
    Триада CIA представляет три фундаментальных принципа информационной безопасности: Конфиденциальность (защита данных от несанкционированного доступа), Целостность (обеспечение точности и подлинности данных) и Доступность (обеспечение доступности систем и данных при необходимости).
  </BaseQuizAnswer>
</BaseQuiz>
...
<BaseQuiz id="comptia-crypto-1" correct="B">
  <template #question>
    Какова основная разница между симметричным и асимметричным шифрованием?
  </template>
  
  <BaseQuizOption value="A">Симметричное быстрее, асимметричное медленнее</BaseQuizOption>
  <BaseQuizOption value="B" correct>Симметричное использует один ключ для шифрования/дешифрования, асимметричное использует пару ключей</BaseQuizOption>
  <BaseQuizOption value="C">Симметричное для электронной почты, асимметричное для файлов</BaseQuizOption>
  <BaseQuizOption value="D">Разницы нет</BaseQuizOption>
  
  <BaseQuizAnswer>
    Симметричное шифрование использует один и тот же ключ для шифрования и дешифрования, что делает его быстрее, но требует безопасного распределения ключей. Асимметричное шифрование использует пару открытого/закрытого ключей, решая проблему распределения ключей, но являясь более вычислительно затратным.
  </BaseQuizAnswer>
</BaseQuiz>

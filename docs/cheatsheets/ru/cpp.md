...
<BaseQuiz id="cpp-main-1" correct="B">
  ...
  <BaseQuizAnswer>
    C использует <code>printf()</code> из stdio.h, в то время как C++ использует <code>cout</code> из iostream с оператором вставки потока <code><<</code>. C++ также поддерживает printf для совместимости.
  </BaseQuizAnswer>
</BaseQuiz>
...
<BaseQuiz id="cpp-vector-1" correct="B">
  <template #question>
    В чем основное преимущество <code>vector</code> перед обычными массивами в C++?
  </template>
  ...
  <BaseQuizAnswer>
    <code>vector</code> — это динамический массив, который может расти или уменьшаться во время выполнения, в отличие от обычных массивов, размер которых фиксируется на этапе компиляции. Это делает векторы более гибкими для многих сценариев использования.
  </BaseQuizAnswer>
</BaseQuiz>
...
<BaseQuiz id="cpp-range-for-1" correct="B">
  ...
  <BaseQuizAnswer>
    Циклы for на основе диапазона (введенные в C++11) автоматически перебирают все элементы в контейнере (например, векторах, массивах, строках) без необходимости вручную управлять индексами. Синтаксис: <code>for (auto item : container)</code>.
  </BaseQuizAnswer>
</BaseQuiz>

---
title: 'Nmap チートシート'
description: 'Nmap の必須コマンド、概念、ベストプラクティスを網羅した包括的なチートシートで Nmap を習得しましょう。'
...
ハンズオンラボと実世界のシナリオを通じて Nmap ネットワークスキャンを学びましょう。LabEx は、必須のネットワークディスカバリ、ポートスキャン、サービス検出、OS フィンガープリント、脆弱性評価を網羅した包括的な Nmap コースを提供します。ネットワーク偵察とセキュリティ監査の技術を習得してください。
</base-disclaimer-content>
</base-disclaimer>
...
Nmap はホスト名、IP アドレス、サブネットをサポートします。
...
ホストディスカバリは、多くのアナリストやペンテスターが Nmap を使用する主要な方法です。その目的は、どのシステムがオンラインであるかの概要を把握することです。
...
ローカルネットワークのディスカバリに ARP リクエストを使用します。
...
### TCP SYN スキャン：`nmap -sS`

このスキャンはステルス性が高く、Nmap が RST パケットを送信するため、複数のリクエストを防ぎ、スキャン時間を短縮します。
...
### TCP Connect スキャン：`nmap -sT`

Nmap は SYN フラグが設定された TCP パケットをポートに送信します。これにより、ポートが開いているか、閉じているか、不明かを知ることができます。
...
サービスの UDP ポートをスキャンします。

```bash
# UDPスキャン（遅い、root権限が必要）
nmap -sU 192.168.1.1
# 一般的なUDPポートのスキャン
...
より正確なスキャンのために、特定のポート、範囲、または TCP と UDP の組み合わせをターゲットにします。
...
実行中のサービスを検出し、そのソフトウェアのバージョンと設定を特定しようとします。
...
TCP/IPフィンガープリントを使用して、ターゲットホストのオペレーティングシステムを推測します。
...
ターゲット環境と検出リスクに基づいてスキャン速度を調整します。

```bash
# パラノイア（非常に遅い、ステルス性が高い）
nmap -T0 192.168.1.1
# スニーキー（遅い、ステルス性が高い）
nmap -T1 192.168.1.1
# 丁寧（遅い、帯域幅が少ない）
...
# アグレッシブ（速い）
nmap -T4 192.168.1.1
# クレイジー（非常に速い、結果を見逃す可能性あり）
nmap -T5 192.168.1.1
```

### カスタムタイミングオプション

パフォーマンスを最適化するために、タイムアウト、再試行、並列スキャンに関する設定を微調整します。

```bash
# 最小レート（1秒あたりのパケット数）を設定
...
Nmap の出力をコマンドラインツールと組み合わせて、有用な洞察を抽出します。
...
パケットの断片化、IP アドレスの偽装、ステルススキャン手法を使用してセキュリティ対策を回避します。
...
Nmap が DNS ルックアップをどのように処理するかを制御します。
...
IPv6 サポートなどの追加機能には、これらの Nmap フラグを使用します。
...
以下の例は、ターゲットネットワーク上の Netbios を列挙します。

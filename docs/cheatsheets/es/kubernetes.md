---
title: 'Lista Rápida de Kubernetes'
description: 'Aprenda Kubernetes con nuestra hoja de trucos completa que cubre comandos esenciales, conceptos y mejores prácticas.'
...
<a target="_blank" href="https://labex.io/es/learn/kubernetes">Aprenda Kubernetes con Laboratorios Prácticos</a>
</base-disclaimer-title>
<base-disclaimer-content>
Aprenda la orquestación de contenedores de Kubernetes a través de laboratorios prácticos y escenarios del mundo real. LabEx proporciona cursos completos de Kubernetes que cubren los comandos esenciales de kubectl, gestión de pods, implementaciones, servicios, redes y administración de clústeres. Domine la orquestación de contenedores y el despliegue de aplicaciones nativas de la nube.
</base-disclaimer-content>
</base-disclaimer>
...
# Comprobar las versiones del cliente y del servidor
kubectl version
...
# Establecer el espacio de nombres predeterminado
...
# Mostrar versiones de API compatibles
...
### Operaciones de Espacio de Nombres: `kubectl get namespaces`

Organizar y aislar recursos mediante espacios de nombres.
...
namespace
# Obtener recursos en un espacio de nombres específico
...
### Ver Información del Pod: `kubectl get pods`
...
# Listar pods en todos los espacios de nombres
...
# Describir pod en un espacio de nombres específico
...
### Crear Despliegues: `kubectl create deployment`
...
# Crear despliegue con réplicas
kubectl create deployment webapp --image=nginx --
replicas=3
# Crear desde archivo YAML
kubectl apply -f deployment.yaml
# Exponer despliegue como servicio
kubectl expose deployment nginx --port=80 --
type=LoadBalancer
```

### Gestionar Despliegues: `kubectl get deployments`

Ver y controlar el estado y la configuración del despliegue.
...
# Autoescalar despliegue
kubectl autoscale deployment nginx --min=2 --max=10 --
cpu-percent=80
```

### Actualizaciones Progresivas: `kubectl rollout`

Gestionar actualizaciones y reversiones de despliegues.
...
```bash
# Exponer despliegue como servicio ClusterIP
kubectl expose deployment nginx --port=80
...
### Reenvío de Puertos: `kubectl port-forward`
...
from-file=tls.key
# Crear secreto de registro de docker
...
### Volúmenes Persistentes: `kubectl get pv`

Gestionar recursos de almacenamiento en todo el clúster.

```bash
# Listar volúmenes persistentes
kubectl get pv
# Describir volumen persistente
kubectl describe pv
# Crear PV desde YAML
kubectl apply -f persistent-volume.yaml
# Eliminar volumen persistente
kubectl delete pv
```

### Solicitudes de Volumen Persistente: `kubectl get pvc`
...
### Clases de Almacenamiento: `kubectl get storageclass`
...
Monitorizar el consumo de recursos en clústeres y nodos.
...
kubectl top pods
# Ver uso de recursos del pod en el espacio de nombres
kubectl top pods -n
...
```bash
# Obtener todos los recursos en el espacio de nombres
kubectl get all
...
Comparar configuraciones y entender esquemas de recursos.
...
Gestionar la disponibilidad del nodo para mantenimiento y actualizaciones.
...
Acceder a las API del clúster y gestionar la autenticación.
...
sa
# Ver información de autenticación del usuario
...
run=client -o yaml
# Ordenar recursos por tiempo de creación
...
# Uso de recursos históricos (requiere metrics-server)
kubectl top pods --previous
```

### Comprobaciones de Estado

Monitorizar la salud de las aplicaciones y el clúster.

```bash
# Comprobar estado del rollout del despliegue
kubectl rollout status deployment/
# Comprobar la preparación del pod
kubectl get pods --field-selector=status.phase=Running
# Monitorizar cuotas de recursos
...
```bash
# Ver solicitudes y límites de recursos
kubectl describe node  | grep -A5 "Allocated resources:"
# Comprobar presupuestos de interrupción de pod
kubectl get pdb
# Ver autoescaladores horizontales de pod
kubectl get hpa
...
```bash
# Copia de seguridad de todos los recursos en el espacio de nombres
...
Establecer espacios de nombres predeterminados y preferencias para las operaciones de kubectl.

```bash
# Establecer espacio de nombres predeterminado para el
contexto actual
...
Personalizar la salida de comandos para una mejor legibilidad y procesamiento.
...
# Mostrar diferencias antes de aplicar
kubectl diff -f deployment.yaml
# Forzar eliminación con período de gracia
kubectl delete pod  --grace-period=0 --force
```

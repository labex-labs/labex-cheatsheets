---
title: 'Matplotlib 速查表'
description: '使用我们涵盖基本命令、概念和最佳实践的综合速查表，快速掌握 Matplotlib。'
...
<a target="_blank" href="https://labex.io/zh/learn/matplotlib">通过实践实验室学习 Matplotlib</a>
</base-disclaimer-title>
...
plt.plot(x, [1, 3, 5, 7, 9], label='线 2')
plt.legend()
...
plt.bar(x - 0.2, values, 0.4, label='组 1')
plt.bar(x + 0.2, [15, 25, 35, 20], 0.4, label='组 2')
```
...
# 多个直方图
plt.hist([data1, data2], bins=30, alpha=0.7, label=['数据 1', '数据 2'])
```
...
# 自定义箱线图
plt.boxplot(data, labels=['组 1', '组 2', '组 3', '组 4'],
...
ax3 = fig.add_subplot(gs[1:, -1])  # 最后一列，底下的 2 行
...
plt.plot(x, y, ax=axes[0,1])  # 纯粹的 matplotlib
```

### Jupyter 集成：内联绘图

为 Jupyter notebook 环境优化 Matplotlib。
